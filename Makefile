...
out/WindowsSucks.dll: src/WindowsSucks.cpp src/WindowsSucks.h
	$(CXX) $(CXXFLAGS) -shared -o out/WindowsSucks.dll src/WindowsSucks.cpp -Wl,--out-implib,out/libWindowsSucks.a

out/run-or-raise.exe: src/main.cpp out/WindowsSucks.dll
	$(CXX) $(CXXFLAGS) -static-libgcc -static-libstdc++ src/main.cpp -o out/run-or-raise.exe -Lout -lWindowsSucks
